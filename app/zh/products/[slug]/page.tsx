...
export default async function ChineseProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      {product.gallery.length > 0 && <section className="gallery-showcase"><div className="product-gallery-heading shell"><div><p className="eyebrow">应用与细节</p><h2>为真实空间<br />而设计。</h2></div><p>产品效果图、展厅样品与已完成项目，按照所对应的系统集中展示。</p></div><div className={`product-gallery product-gallery--${product.gallery.length}`}>{product.gallery.map((image, index) => <figure key={image.src} className={product.gallery.length > 2 && index === 0 ? "gallery-feature" : ""}><img src={image.src} alt={image.alt} loading="lazy" decoding="async" /><figcaption><span>{String(index + 1).padStart(2, "0")}</span>{image.caption}</figcaption></figure>)}</div></section>}
